    ...
    <div className="flex h-screen bg-base">
      ...
      <aside className="w-[240px] shrink-0 bg-surface border-r border-border flex flex-col">
        ...
        <nav className="flex-1 px-3 space-y-0.5" role="navigation">
          ...
          {nav("/", "Dashboard", <Icon d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />)}
          {nav("/chat", "Ask Memory", <Icon d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />)}
          {nav("/search", "Search", <Icon d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />)}

          <div className="text-[10px] text-text-disabled font-medium uppercase tracking-[0.08em] px-2 pt-4 pb-1">System</div>
          {nav("/settings", "Settings", <Icon d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" />)}
        </nav>
        ...
        <div className="px-4 py-3 border-t border-border">
          ...
          >
            {captureActive
              ? <><span className="dot-pulse" /><span className="text-text-secondary">Capturing</span></>
              : <><span className="w-2 h-2 rounded-full bg-text-disabled" /><span className="text-text-disabled">Paused</span></>
            }
          </button>
        </div>
      </aside>
