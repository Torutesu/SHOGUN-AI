...
type IconName =
  | "dashboard" | "memory" | "chat" | "agents" | "work" | "capture"
  | "plug" | "settings" | "search" | "plus" | "more" | "arrowRight"
  | "arrowLeft" | "arrowUpRight" | "check" | "clock" | "zap" | "bot"
  | "mail" | "calendar" | "slack" | "file" | "note" | "moon"
  | "chevronRight" | "chevronDown" | "database" | "shield" | "key"
  | "play" | "pause" | "filter" | "link" | "terminal" | "tag" | "pin"
  | "paperclip" | "mic" | "users" | "grid" | "star" | "upload"
  | "popout" | "globe" | "lock" | "x" | "logout" | "download" | "gift"
  | "keyboard" | "edit" | "eye" | "info";
...
const PATHS: Record<string, React.ReactNode> = {
  ...
  slack: (<><rect x="3" y="10" width="4" height="4"/><rect x="10" y="3" width="4" height="4"/><rect x="17" y="10" width="4" height="4"/><rect x="10" y="17" width="4" height="4"/></>),
  ...
  gift: (<><rect x="3" y="8" width="18" height="5" rx="1"/><path d="M12 8v13M5 13v8h14v-8"/></>),
...
export function Icon({ name, size = 16, className = "" }: { name: IconName | string; size?: number; className?: string }) {
  return (
    <svg className={`ico ${className}`} width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      {PATHS[name] || null}
    </svg>
  );
}

export function Kamon({ size = 28, color = "currentColor" }: { size?: number; color?: string }) {
  return (
